...
const CategoryCard: React.FC<CategoryCardProps> = ({ category, progress, onClick }) => {
    ...
    return (
        <div 
            onClick={onClick}
            className={`category-card group bg-dark/50 backdrop-blur-lg border ${colorClasses.border} rounded-lg p-6 flex flex-col transition-all duration-300 ${colorClasses.shadow} transform hover:-translate-y-2 cursor-pointer relative overflow-hidden`}
        >
